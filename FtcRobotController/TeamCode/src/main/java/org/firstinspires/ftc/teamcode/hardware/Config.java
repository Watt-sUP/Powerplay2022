package org.firstinspires.ftc.teamcode.hardware;

public class Config {
    public static final String left_front = "RF";
    public static final String right_front = "LF";
    public static final String left_back = "RB";
    public static final String right_back = "LB";

    public static final String glisiera = "GLI";
    public static final String turela = "TUR";
    public static final String deget = "DEG";
    public static final String glisiera1 = "GLI1";
    public static final String odometry_servo = "ODO";

    public static final String VuforiaKey = "";
}
